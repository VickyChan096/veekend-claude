...
## Phase 11 — 內文版型速查對話框（2026-08-19）

### 做了什麼

編輯頁填內文區塊時，要靠想像才知道每種版面長什麼樣。加一個用色塊示意的速查對話框，入口在「內文」段落標題旁的「版型速查」按鈕。

示意圖用純 CSS 畫、不放真圖——目的是看懂「元素怎麼排」而不是看內容：

- **灰塊** = 圖片
- **黃條** = 標題
- **淺灰線** = 文字

每個版型旁邊標了實際使用次數，一眼看出哪些是主力。
...
### 統整時發現的事：兩種版面在桌機版其實一樣

對照 CSS 才確認：**`imageFirst` 與 `textFirst` 在電腦版的排法完全相同**，兩者都只是照 `parts` 的順序往下排。唯一的 CSS 差異是 `textFirst` 在**手機版**會 `column-reverse`，把圖片移到文字上方。

也就是說：**實際順序是編輯的人排出來的，版面值只影響手機版。** 選了「文字在上」但把圖片放在元素清單第一個，電腦版看起來就跟「圖在上」一樣。

資料也印證了這個用法——`imageLeft` 的 12 個全都是 `image → h4 → h6 → list`，`imageRight` 的 11 個全都是 `h4 → h6 → list → image`，順序是刻意排的。

這點寫進對話框的「兩件容易搞混的事」，免得選了版面卻覺得沒反應。

順手清掉 `ArticleSection` 裡一段重複的 `margin-bottom` 規則，並補上註解說明兩種版面的真正差異。

### 踩到的坑：空的 slot 會讓 Vue 回退到預設內容

`BaseDialog` 的 footer 寫法是：

```vue
<footer v-if="defaultButtons || $slots.footer">
  <slot name="footer">預設的取消／確定按鈕</slot>
</footer>
```

我原本傳了一個空的 `#footer` 想蓋掉按鈕，結果 **Vue 判定「這個 slot 渲染不出東西」就回退到 fallback**，反而把按鈕叫出來了。

要沒有按鈕就得**完全不傳這個 slot**，只設 `:default-buttons="false"`。已在元件註解標明，免得日後又加回去。

### 又一次的截圖假象

截圖看起來對話框背景是半透明的、底下表單透出來。但用三種方式確認都顯示它是實心的：

1. computed `backgroundColor` = `rgb(255, 255, 255)`
2. 整條祖先鏈的 `opacity` 都是 1
3. `elementFromPoint` 打在對話框中心，命中的是對話框內的元素

結論是截圖合成的假象，跟 Phase 9 那次「圖片不載入」一樣——**瀏覽器工具的呈現不等於真實狀態**。這次因為有前車之鑑，很快就轉去查 computed styles 而不是改程式。

### 驗證結果

- 瀏覽器實測：開啟對話框、六種版型都正確渲染、底部無多餘按鈕
